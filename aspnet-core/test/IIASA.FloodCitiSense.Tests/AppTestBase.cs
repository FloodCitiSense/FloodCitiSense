...
using IIASA.FloodCitiSense.Tests.TestDatas;
...
namespace IIASA.FloodCitiSense.Tests
{
    ...
    /// Provides methods to easily work with <see cref="FloodCitiSenseDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<FloodCitiSenseTestModule>
    {
        ...
        {
            void NormalizeDbContext(FloodCitiSenseDbContext context)
            {
        ...
        protected void UsingDbContext(Action<FloodCitiSenseDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<FloodCitiSenseDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<FloodCitiSenseDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<FloodCitiSenseDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<FloodCitiSenseDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FloodCitiSenseDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<FloodCitiSenseDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FloodCitiSenseDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<FloodCitiSenseDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<FloodCitiSenseDbContext, Task<T>> func)
        {
            T result;
