...
using IIASA.FloodCitiSense.Web.Authentication.TwoFactor;
using IIASA.FloodCitiSense.Web.Configuration;
#if FEATURE_SIGNALR
using Abp.Web.SignalR;
#endif

namespace IIASA.FloodCitiSense.Web
{
    [DependsOn(
    ...
    public class FloodCitiSenseWebCoreModule : AbpModule
    {
        private readonly IHostingEnvironment _env;
        private readonly IConfigurationRoot _appConfiguration;

        public FloodCitiSenseWebCoreModule(IHostingEnvironment env)
        {
            _env = env;
        ...
        public override void PreInitialize()
        {
            ...
            Configuration.Modules.AbpAspNetCore()
                .CreateControllersForAppServices(
                    typeof(FloodCitiSenseApplicationModule).GetAssembly()
                );
        ...
        {
            IocManager.RegisterAssemblyByConvention(typeof(FloodCitiSenseWebCoreModule).GetAssembly());
        }
        ...
        private void SetAppFolders()
        {
            ...
            appFolders.ImagesFolder = Path.Combine(_env.ContentRootPath, $"App_Data{Path.DirectorySeparatorChar}Images");
            appFolders.FilesFolder = Path.Combine(_env.ContentRootPath, $"App_Data{Path.DirectorySeparatorChar}Files");

#if NET461
            if (_env.IsDevelopment())
            {
                var currentAssemblyDirectoryPath = typeof(FloodCitiSenseWebCoreModule).GetAssembly().GetDirectoryPathOrNull();
            ...
            try
            {
                ...
                DirectoryHelper.CreateIfNotExists(appFolders.ImagesFolder);
                DirectoryHelper.CreateIfNotExists(appFolders.FilesFolder);
            }
            catch { }
        }
    }
